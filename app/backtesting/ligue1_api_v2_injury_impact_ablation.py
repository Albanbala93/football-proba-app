"""Test whether weighted injury features improve Ligue 1 API xG v2."""
...
DEFAULT_INPUT_PATH = PROJECT_ROOT / "data" / "processed" / "api_football_ligue1_2025_injury_impact_features.csv"
DEFAULT_REPORT_PATH = PROJECT_ROOT / "data" / "predictions" / "ligue1_api_v2_injury_impact_ablation_report.csv"
DEFAULT_PREDICTIONS_PATH = PROJECT_ROOT / "data" / "predictions" / "ligue1_api_v2_injury_impact_ablation_predictions.csv"
...
CONFIGS = [
    ModelConfig("base_v2", BASE_V2_FEATURES),
    ModelConfig("base_v2_plus_injury", BASE_V2_FEATURES + INJURY_FEATURES),
]
...
def load_data(input_path: Path) -> pd.DataFrame:
    """Load injury impact rows and keep scored Ligue 1 matches."""
    if not input_path.exists():
        raise FileNotFoundError(f"Input file not found: {input_path}")

    data = pd.read_csv(input_path)
    required = ["date", "fixture_id", "goals_home", "goals_away", *sorted(set(BASE_V2_FEATURES + INJURY_FEATURES))]
...
def run_ablation(input_path: Path = DEFAULT_INPUT_PATH, report_path: Path = DEFAULT_REPORT_PATH, predictions_path: Path = DEFAULT_PREDICTIONS_PATH) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def print_summary(summary: pd.DataFrame, report_path: Path) -> None:
    ...
    print("Ligue 1 API v2 injury impact ablation")
    ...
    base_row = summary[summary["config"] == "base_v2"].iloc[0]
    injury_row = summary[summary["config"] == "base_v2_plus_injury"].iloc[0]
    delta_accuracy = float(injury_row["accuracy"] - base_row["accuracy"])
    delta_log_loss = float(injury_row["log_loss"] - base_row["log_loss"])
    delta_brier = float(injury_row["brier_score_multiclass"] - base_row["brier_score_multiclass"])
    print("\nComparison base_v2 -> base_v2_plus_injury")
    ...
    if delta_log_loss < 0 and delta_brier < 0:
        decision = "Integrate injury features: both log_loss and Brier improved."
    else:
        decision = "Do not integrate injury features yet: no simultaneous improvement on log_loss and Brier."
    print(f"\nDecision: {decision}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run Ligue 1 API xG v2 injury impact ablation.")
