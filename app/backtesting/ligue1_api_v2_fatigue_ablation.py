"""Test whether fatigue / calendar features improve Ligue 1 API xG v2 with injury impact."""
...
DEFAULT_INPUT_PATH = PROJECT_ROOT / "data" / "processed" / "api_football_ligue1_2025_fatigue_features.csv"
DEFAULT_REPORT_PATH = PROJECT_ROOT / "data" / "predictions" / "ligue1_api_v2_fatigue_ablation_report.csv"
DEFAULT_PREDICTIONS_PATH = PROJECT_ROOT / "data" / "predictions" / "ligue1_api_v2_fatigue_ablation_predictions.csv"
...
CONFIGS = [
    ...
    ModelConfig("base_plus_fatigue", BASE_CURRENT_FEATURES + FATIGUE_FEATURES),
]
...
def load_data(input_path: Path) -> pd.DataFrame:
    """Load fatigue rows and keep scored Ligue 1 matches."""
    if not input_path.exists():
        raise FileNotFoundError(f"Input file not found: {input_path}")

    data = pd.read_csv(input_path)
    required = ["date", "fixture_id", "goals_home", "goals_away", *sorted(set(BASE_CURRENT_FEATURES + FATIGUE_FEATURES))]
...
def print_summary(summary: pd.DataFrame, report_path: Path) -> None:
    ...
    print("Ligue 1 API v2 fatigue ablation")
    ...
    fatigue_row = summary[summary["config"] == "base_plus_fatigue"].iloc[0]
    delta_accuracy = float(fatigue_row["accuracy"] - base_row["accuracy"])
    delta_log_loss = float(fatigue_row["log_loss"] - base_row["log_loss"])
    delta_brier = float(fatigue_row["brier_score_multiclass"] - base_row["brier_score_multiclass"])
    print("\nComparison base_current -> base_plus_fatigue")
    ...
    if delta_log_loss < 0 and delta_brier < 0:
        decision = "Integrate fatigue features: both log_loss and Brier improved."
    else:
        decision = "Do not integrate fatigue features yet: no simultaneous improvement on log_loss and Brier."
    print(f"\nDecision: {decision}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run Ligue 1 API xG v2 fatigue ablation.")
